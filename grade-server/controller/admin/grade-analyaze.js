...
class AdminGradeAnalyze {
  async uploadManyClass(req, res) {
    ...
    try {
      ...
      const documents = data.map((item) => {
        ...
        const score = toNumber(item["语文"]) + toNumber(item["数学"]) + toNumber(item["英语"]) + toNumber(item["道法"]) * pointFive + toNumber(item["历史"]) * pointFive + toNumber(item["地理"]) * pointFive + toNumber(item["生物"]) * pointFive + toNumber(item["物理"]) * pointSeven + toNumber(item["化学"]) * pointFive;
  ...
  async postAnalyzeParams(req, res) {
    try {
      const { takePartinNum, inLineRate, excellentRate, numOfPeople, examId } = req.body;
      ...
      const subjects = ["chinese", "math", "english", "political", "history", "geography", "biology", "physics", "chemistry"];
      subjects.forEach((subject) => {
        const sortedGrades = [...grades].sort((a, b) => b[subject] - a[subject]);
        let inLineCount = Math.floor(numOfPeople * inLineRate * 0.01);
        let excellentCount = Math.floor(numOfPeople * excellentRate * 0.01);

        // 处理重分情况，确定实际进线分数线
        let inLineScore = sortedGrades[inLineCount - 1]? sortedGrades[inLineCount - 1][subject] : 0;
        while (inLineCount < sortedGrades.length && sortedGrades[inLineCount][subject] === inLineScore) {
          inLineCount++;
        }
        inLineScores[subject] = inLineScore;

        // 处理重分情况，确定实际优秀分数线
        let excellentScore = sortedGrades[excellentCount - 1]? sortedGrades[excellentCount - 1][subject] : 0;
        while (excellentCount < sortedGrades.length && sortedGrades[excellentCount][subject] === excellentScore) {
          excellentCount++;
        }
      ...
      let totalInLineScore = sortedTotalGrades[totalInLineCount - 1]? sortedTotalGrades[totalInLineCount - 1].score : 0;
      while (totalInLineCount < sortedTotalGrades.length && sortedTotalGrades[totalInLineCount].score === totalInLineScore) {
        totalInLineCount++;
      }

      // 处理总分重分情况，确定实际总分优秀分数线
      let totalExcellentScore = sortedTotalGrades[totalExcellentCount - 1]? sortedTotalGrades[totalExcellentCount - 1].score : 0;
      while (totalExcellentCount < sortedTotalGrades.length && sortedTotalGrades[totalExcellentCount].score === totalExcellentScore) {
        totalExcellentCount++;
      }
      ...
      for (const className in classGroups) {
        ...
        subjects.forEach((subject) => {
          // 按当前科目成绩排序并取前 n 名
          const sortedStudents = [...classStudents].sort((a, b) => b[subject] - a[subject]);
          const topStudents = sortedStudents.slice(0, numOfParticipants);

          // 计算当前科目平均分
          const subjectScoreSum = topStudents.reduce((sum, student) => sum + student[subject], 0);
          subjectAverages[subject] = subjectScoreSum / numOfParticipants;

          // 计算进线人数和进线率
          const inLineStudentIds = classStudents.filter(student => student[subject] >= inLineScores[subject]).map(student => student._id);
          ...
          // 计算优秀人数和优秀率
          const excellentStudentIds = classStudents.filter(student => student[subject] >= excellentScores[subject]).map(student => student._id);
          let excellentCount = excellentStudentIds.length;
          const excellentRateClass = classSize > 0 ? excellentCount / classSize : 0;
        ...
        const topStudents = sortedStudents.slice(0, numOfParticipants);
        const totalScoreSum = topStudents.reduce((sum, student) => sum + student.score, 0);
        subjectAverages.totalScore = totalScoreSum / numOfParticipants;

        // 计算总分的进线人数和进线率
        const totalInLineStudentIds = classStudents.filter(student => student.score >= totalInLineScore).map(student => student._id);
        const totalInLineCount = totalInLineStudentIds.length;
        const totalInLineRate = classSize > 0 ? totalInLineCount / classSize : 0;
        subjectAverages.totalInLineCount = totalInLineCount;
        subjectAverages.totalInLineRate = totalInLineRate;

        // 计算总分的优秀人数和优秀率
        const totalExcellentStudentIds = classStudents.filter(student => student.score >= totalExcellentScore).map(student => student._id);
        const totalExcellentCount = totalExcellentStudentIds.length;
        const totalExcellentRate = classSize > 0 ? totalExcellentCount / classSize : 0;
      ...
      subjects.forEach((subject) => {
        const sortedResultsByAverage = [...classAnalysisResults].sort((a, b) => b.subjectAverages[subject] - a.subjectAverages[subject]);
        sortedResultsByAverage.forEach((result, index) => {
          result.subjectRanks[subject] = index + 1;
        });

        const sortedResultsByInLineRate = [...classAnalysisResults].sort((a, b) => b.subjectAverages[`${subject}InLineRate`] - a.subjectAverages[`${subject}InLineRate`]);
        sortedResultsByInLineRate.forEach((result, index) => {
          result.subjectRanks[`${subject}InLineRateRank`] = index + 1;
        });

        const sortedResultsByExcellentRate = [...classAnalysisResults].sort((a, b) => b.subjectAverages[`${subject}ExcellentRate`] - a.subjectAverages[`${subject}ExcellentRate`]);
      ...
      // 对总分平均分进行排名
      const sortedResultsByTotalScore = [...classAnalysisResults].sort((a, b) => b.subjectAverages.totalScore - a.subjectAverages.totalScore);
      sortedResultsByTotalScore.forEach((result, index) => {
        result.subjectRanks.totalScore = index + 1;
      });

      // 对总分进线率进行排名
      const sortedResultsByTotalInLineRate = [...classAnalysisResults].sort((a, b) => b.subjectAverages.totalInLineRate - a.subjectAverages.totalInLineRate);
      sortedResultsByTotalInLineRate.forEach((result, index) => {
        result.subjectRanks.totalInLineRateRank = index + 1;
      });

      // 对总分优秀率进行排名
      const sortedResultsByTotalExcellentRate = [...classAnalysisResults].sort((a, b) => b.subjectAverages.totalExcellentRate - a.subjectAverages.totalExcellentRate);
      ...
      subjects.forEach((subject) => {
        ...
        classAnalysisResults.forEach((result) => {
          const averageRankScore = (numOfClasses + 1) - result.subjectRanks[subject];
          const inLineRateRankScore = (numOfClasses + 1) - result.subjectRanks[`${subject}InLineRateRank`];
          const excellentRateRankScore = (numOfClasses + 1) - result.subjectRanks[`${subject}ExcellentRateRank`];
          result.subjectAverages[`${subject}TotalScore`] = averageRankScore * 0.6 + inLineRateRankScore * 0.2 + excellentRateRankScore * 0.2;
        });

        // 对该学科总评得分进行排序
        const sortedByTotalScore = [...classAnalysisResults].sort((a, b) => b.subjectAverages[`${subject}TotalScore`] - a.subjectAverages[`${subject}TotalScore`]);
      ...
      // 计算总分总评得分和总评名次
      classAnalysisResults.forEach((result) => {
        const totalAverageRankScore = (numOfClasses + 1) - result.subjectRanks.totalScore;
        const totalInLineRateRankScore = (numOfClasses + 1) - result.subjectRanks.totalInLineRateRank;
        const totalExcellentRateRankScore = (numOfClasses + 1) - result.subjectRanks.totalExcellentRateRank;
        result.subjectAverages.totalTotalScore = totalAverageRankScore * 0.6 + totalInLineRateRankScore * 0.2 + totalExcellentRateRankScore * 0.2;
      });

      const sortedByTotalTotalScore = [...classAnalysisResults].sort((a, b) => b.subjectAverages.totalTotalScore - a.subjectAverages.totalTotalScore);
      sortedByTotalTotalScore.forEach((result, index) => {
        result.subjectRanks.totalTotalRank = index + 1;
      });

      // 保存分析结果到数据库
      const analyzeGradeResult = await ClassGradeAnalysis.insertMany(classAnalysisResults);
  ...
  async getAnalyzeResults(req, res) {
